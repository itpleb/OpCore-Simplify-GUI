...
patches = [
    PatchInfo(
        name = "ALS",
        description = "模拟或启用环境光传感器设备，用于存储当前亮度/自动亮度级别",
        function_name = "ambient_light_sensor"
    ),
    PatchInfo(
        name = "APIC",
        description = "在 HEDT 系统上，通过将第一个 CPU 条目指向活动 CPU 来避免内核崩溃",
    ...
    PatchInfo(
        ...
        description = "禁用不支持的 PCI 设备，如 GPU、Wi-Fi 卡和 SD 读卡器",
        function_name = "disable_unsupported_device"
    ),
    PatchInfo(
        name = "FakeEC",
        description = "操作系统感知的虚拟 EC (by CorpNewt)",
        function_name = "fake_embedded_controller"
    ),
    PatchInfo(
        name = "RCSP",
        description = "移除条件 ACPI 作用域声明",
        function_name = "remove_conditional_scope"
    ),
    PatchInfo(
        name = "CMOS",
        description = "修复 HP 实时时钟掉电 (005) POST 错误",
        function_name = "fix_hp_005_post_error"
    ),
    PatchInfo(
        name = "FixHPET",
        description = "修复 IRQ 冲突 (by CorpNewt)",
        function_name = "fix_irq_conflicts"
    ),
    PatchInfo(
        name = "GPI0",
        description = "为 I2C 触摸板启用 GPIO 设备以正常工作",
        function_name = "enable_gpio_device"
    ),
    PatchInfo(
        name = "IMEI",
        description = "创建虚拟 IMEI 设备以确保 Intel iGPU 加速功能正常工作",
    ...
    PatchInfo(
        name = "PMC",
        description = "添加 PMCR 设备以启用 300 系列主板的 NVRAM 支持",
        function_name = "enable_nvram_support"
    ),
    PatchInfo(
        name = "PM (Legacy)",
        description = "阻止 CpuPm 和 Cpu0Ist ACPI 表以避免 Intel Ivy Bridge 和更老的 CPU 崩溃",
        function_name = "drop_cpu_tables"
    ),
    PatchInfo(
        name = "PLUG",
        description = "将 CPU 对象重新定义为 Processor 并设置 plugin-type = 1 (by CorpNewt)",
    ...
    PatchInfo(
        name = "RMNE",
        description = "创建空以太网以允许 macOS 系统访问 iServices",
    ...
    PatchInfo(
        name = "RTCAWAC",
        description = "上下文感知的 AWAC 禁用和 RTC 启用/模拟/范围修复 (by CorpNewt)",
        function_name = "fix_system_clock_awac"
    ),
    PatchInfo(
        name = "PRW",
        description = "修复 _PRW 方法中的睡眠状态值以防止在 macOS 中立即唤醒",
        function_name = "instant_wake_fix"
    ),
    PatchInfo(
        name = "Surface Patch",
        description = "所有 Surface Pro / Book / Laptop 硬件的特殊补丁",
        function_name = "surface_laptop_special_patch"
    ),
    PatchInfo(
        name = "UNC",
        description = "禁用未使用的非核心桥以防止 HEDT 系统上的内核崩溃",
    ...
    PatchInfo(
        name = "XOSI",
        description = "将操作系统模拟为 Windows，在 macOS 上启用锁定在非 Windows 系统后面的设备",
