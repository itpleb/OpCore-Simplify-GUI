...
class HardwareCustomizer:
    ...
    def show_macos_compatibility(self, device_compatibility):
        ...
        if max_compatibility == min_version:
            return "<span style='color:blue'>最高支持至 {}</span>".format(
        ...
        for device_type, devices in self.hardware_report.items():
            ...
            if device_type == "BIOS":
                ...
                if devices.get("Firmware Type") != "UEFI":
                    content = (
                        "您想要构建 UEFI 的 EFI 吗？<br>"
                        "如果选择是，请确保更新 BIOS 并在 BIOS 设置中启用 UEFI 启动模式。<br>"
                        "如果需要，您也可以继续使用 Legacy 模式。"
                    )
                    if show_confirmation("BIOS 固件类型不是 UEFI", content):
                        self.utils.log_message("[HARDWARE CUSTOMIZATION] BIOS 固件类型不是 UEFI，正在为 UEFI 构建 EFI", level="INFO")
                        self.customized_hardware[device_type]["Firmware Type"] = "UEFI"
                    else:
                        self.utils.log_message("[HARDWARE CUSTOMIZATION] BIOS 固件类型不是 UEFI，正在为 Legacy 构建 EFI", level="INFO")
    ...
    def _handle_device_selection(self, device_type):
        ...
        if len(devices) > 1:       
            if device_type == "WiFi" or device_type == "Bluetooth":
                content.append("macOS 最好只启用一个 {} 设备。<br>".format(device_type))
                ...
                if _apu_index or _navi_22_indices:
                    content.append("多个活动的 GPU 可能在 macOS 中导致驱动冲突。")
    ...
    def _select_device(self, device_type, devices, device_groups=None, title=None, content=None):
        self.utils.log_message("[HARDWARE CUSTOMIZATION] 正在选择设备：{}".format(device_type), level="INFO")
        if device_groups:
        ...
        if device_groups:
            ...
            for device_name, device_props in devices.items():
                ...
                option += "<br>设备 ID：{}".format(device_props.get("Device ID", "未知"))
        ...
        self.utils.log_message("[HARDWARE CUSTOMIZATION] 正在选择设备：{}".format(", ".join(option.split("<br>")[0].replace("<b>", "").replace("</b>", "").strip() for option in options)), level="INFO")
        ...
        while True:
            ...
            self.utils.log_message("[HARDWARE CUSTOMIZATION] 已选择的设备：{}".format(", ".join(selected_devices)), level="INFO")
